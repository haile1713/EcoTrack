...
import { getGeocodeSuggestions } from '@/utils/db/api';
const geminiApiKey = process.env.GEMINI_API_KEY;
// const googleMapsApiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;

const libraries: Libraries = ['places'];
type LocationSuggestion = {
  formatted: string; // Ensure this matches the structure of the OpenCage response
};
...
export default function ReportPage() {
 ...
 // Define locationSuggestions with the correct type
 const [locationSuggestions, setLocationSuggestions] = useState<LocationSuggestion[]>([]);

 const handleLocationChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
   const query = e.target.value;
   setNewReport({ ...newReport, location: query });

   if (query.length > 2) {
     const suggestions = await getGeocodeSuggestions(query);
 ...
 const handleSuggestionClick = (suggestion: LocationSuggestion) => {
   setNewReport({
     ...newReport,
     location: suggestion.formatted, // Set the selected location
   });
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <input
        ...
        onChange={handleLocationChange} // Changed to handle location input and suggestions
        required
      ...
      {locationSuggestions.length > 0 && (
        <ul className="border border-gray-300 mt-2 rounded-lg shadow-lg">
            ...
            <tbody className="divide-y divide-gray-200">
              {reports.map((report) => (
                <tr key={report.id} className="hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
  {report.createdAt instanceof Date ? report.createdAt.toLocaleDateString() : report.createdAt}
</td>
